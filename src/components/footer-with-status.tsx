...
export function Footer() {
  ...
  useEffect(() => {
    fetch(
      "http://localhost:3001/?url=https://javaistic.betteruptime.com/badge&theme=light",
    )
      .then((res) => res.json())
            ...
            <div className="mt-2 flex items-center space-x-2 sm:mt-0">
              <span className="relative flex size-2.5">
                <span
                  ...
                  style={{ backgroundColor: statusData.hex, opacity: 0.75 }}
                ></span>
                <span
                  className="relative inline-flex size-2.5 rounded-full"
                  style={{ backgroundColor: statusData.hex }}
                ></span>
              </span>
              <span className="text-xs">{statusData.text}</span>
            </div>
          )}
