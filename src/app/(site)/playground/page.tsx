...
export default function PlayGround(): React.JSX.Element {
  ...
  const runCode = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        // Normalize response shapes from different runners. Prefer
        // stdout when present, otherwise show stderr (compile errors).
        if (data && typeof data === "object") {
          ...
          const stdoutRaw =
            nested && (nested as any).stdout
              ? (nested as any).stdout
              : nested && (nested as any).output
                ? (nested as any).output
                : null;
          const stderrRaw =
          ...
          if (stdoutStr.trim().length > 0) {
            setOutput(stdoutStr.trim());
          } else if (stderrStr.trim().length > 0) {
            setOutput(stderrStr.trim());
          } else if (typeof nested === "string") {
            setOutput(nested.trim());
          } else {
  ...
  return (
    <>
      <div className="m-8 h-full max-h-screen space-y-4">
        <div className="m-auto flex flex-col gap-4 px-2 lg:flex-row lg:items-start">
          <div className="min-h-[50vh] flex-1 rounded-xl border bg-neutral-50 p-2 shadow-sm dark:bg-neutral-900">
